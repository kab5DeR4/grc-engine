...
export const FrameworkCard = memo(function FrameworkCard({
  ...
  const frameworkDomainList = FRAMEWORK_DOMAINS[id] || [
    { name: 'Access Security', score: score || 85 },
    { name: 'Data Protection', score: Math.max((score || 85) - 4, 70) },
    { name: 'System Logging', score: Math.min((score || 85) + 2, 98) },
  ];

  const domains = isAwaiting 
    ? frameworkDomainList.map(d => ({ name: d.name, score: 0 }))
    : frameworkDomainList;
        ...
        <div>
          <div className="flex items-center justify-between gap-2 mb-3">
            ...
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border flex items-center gap-1 ${
              ...
              {isAwaiting ? <AlertCircle size={10} /> : isHealthy ? <ShieldCheck size={10} /> : <AlertCircle size={10} />}
              {isAwaiting ? 'AWAITING' : isHealthy ? 'VERIFIED' : 'ACTION'}
            </span>
          </div>
          ...
          {domains.map((dom, idx) => (
            <div key={idx} className="space-y-1">
              <div className="flex justify-between text-[10px] text-[var(--ink-secondary)] font-semibold">
                ...
                <span className="text-[var(--ink)] tabular-nums">{isAwaiting ? '--%' : `${dom.score}%`}</span>
              </div>
              <div className="w-full bg-[var(--hairline)] h-1.5 rounded-full overflow-hidden">
                <div 
                  className={`h-full rounded-full ${isAwaiting ? 'bg-amber-400' : dom.score >= 85 ? 'bg-[var(--pass)]' : 'bg-[var(--warn)]'}`}
                  style={{ width: `${isAwaiting ? 0 : dom.score}%` }}
                ></div>
              </div>
            </div>
          ))}
